  ...
  return (
    <div> 
      ...
      {items.length===0 ? <h1 className='text-red-500 text-center text-4xl'>NO Product In Cart </h1>:(
        ...
        {items.map((itme)=>(
          <div key={itme.id} className='border rounded-lg p-4 shadow-md flex flex-row items-center justify-between'>
            ...
            <button className='lone text-white px-2 py-1 rounded-full w-7 h-7' onClick={()=>inq(itme.id) }> +</button>
            <h1 className='px-4'>{itme.quantity}</h1>
            <button className='lone text-white px-2 py-1 rounded-full w-7 h-7 ' onClick={()=>deq(itme.id) }> -</button>
            </div>
            <button className='lone text-white px-2 py-1 rounded-md' onClick= {()=> removeItem(itme.id)}> 
              Delete
            </button>
          </div>
